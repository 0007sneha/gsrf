...
function paginationLinks($current_page, $total_pages, $base_url) {
	...
	$html = '<ul class="pagination custom-pagination text-center">';

	if ($current_page == 1) {

		$html .= '<li class="disabled page-item"><a class="page-link" >First</a></li>';
	} else {
	...
	for (; $i <= ($current_page + 4) && ($i <= $total_pages); $i++) {
		($current_page == $i) ? $li_class = ' class="active page-item"' : $li_class = '';
		...
		if ($i == $current_page + 4 && $i < $total_pages) {
			...
			$html = $html . '<li class="disabled page-item"><a class="page-link">...</a></li>';
	...
	if ($current_page == $total_pages) {
		$html .= '<li class="disabled page-item"><a class="page-link">Last</a></li>';
	} else {
...
function CUID(){
	return $_COOKIE['SCUID'] != "" ? filter_var($_COOKIE['SCUID']) : null;
}

function CUIDWithType(){
	return $_COOKIE['SCUID'] != "" ? "SCHA".filter_var($_COOKIE['SCUID']) : null;
}
...
function generatePassword() {
    ...
    for ($i = 0; $i < $length; $i++) {
        $randomSet = $characters[rand(0, count($characters) - 1)];
        $password .= $randomSet[rand(0, strlen($randomSet) - 1)];
    }
